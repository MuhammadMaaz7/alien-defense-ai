...
class UIManager:
    ...
    def setup_ui(self):
        """Initialize all UI elements with safe positioning"""
        # Main panels
...
<b>Distance:</b> {station_data['distance']}px
    ...
    def get_forbidden_zones(self) -> list:
        """Get areas where stations shouldn't be placed"""
    ...
    def add_bomb_effect(self, position: Tuple[int, int]):
        """Create a bombing animation effect at the specified position"""
        bomb_effect = {
            'position': position,
            'time': 0,
            'max_time': 1.0,  # Total duration
            'pulses': [
